 ...
 *
 * <p>Each JAR's classes are relocated under a unique prefix path (e.g. {@code a/}, {@code b/})
 * to avoid name collisions. J2ME library classes are redirected to {@code lib/} shim classes
 * and a handful of collision-prone method names are rewritten.
 */
public class BytecodeProcessor {
        ...
        for (AbstractInsnNode ain : m.instructions) {
            if (ain instanceof TypeInsnNode) {
                ...
                if (ldc.cst instanceof Type) {
                    ...
                    ldc.cst = "L".equals(t.getDescriptor()) || t.getDescriptor().startsWith("L")
     ...
     *
     * <ul>
     *   <li>Classes present in {@link #LIBRARY_MAPPINGS} are redirected to the {@code lib/} shim.</li>
     *   <li>Array descriptors ({@code [Lcom/Foo;}) are remapped element-wise.</li>
     *   <li>{@code java/} and {@code javax/} platform classes are left untouched
     *       (they are provided by the runtime / shimmed separately).</li>
     *   <li>Everything else receives the {@link #prefix}.</li>
     * </ul>
     */
